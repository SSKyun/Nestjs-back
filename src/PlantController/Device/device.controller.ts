...
export class DeviceController {
    ...
    @Req() req:Request): Promise<Device_Stat_Entity>{
        return this.deviceService.createDevice(createDeviceDto,req.user);
    }

    @Get()
    getAllDevice(@Req() req:Request):Promise<Device_Stat_Entity[]>{
        return this.deviceService.getAllDevices(req.user);
    }

    @Patch(':id')
    update(@Param('id')id:number,@Body()deviceEntity:Device_Stat_Entity){

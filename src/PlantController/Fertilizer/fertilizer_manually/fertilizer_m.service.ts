import { Fertilizer_mRepository } from './fertilizer_m.repository';
...
@Injectable()
export class Fertilizer_mService{
    private natsClient : Stan;

    constructor(
        @InjectRepository(Fertilizer_mRepository)
        private fertilizer_mRepository: Fertilizer_mRepository,
      ) {
      ...
      //     console.log(`Connected to NATS at ${natsUrl} - fertilizer_m`);
    ...
    async getuserAll_m(
        ...
        const query = this.fertilizer_mRepository.createQueryBuilder('irrigation_m');
    ...
    createIrrigation_m(
        ...
        return this.fertilizer_mRepository.createIrrigation_m(create_mButtonDto,user);
    }

    async update_manually(id: number, fertilizer_m:Fertilizer_m ): Promise<void> {
        const update = await this.fertilizer_mRepository.findOneBy({ id });
        update.manually_time = fertilizer_m.manually_time;
        update.manually_btn = fertilizer_m.manually_btn;
      
        await this.fertilizer_mRepository.save(update);
